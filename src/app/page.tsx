...
const RandomPage = () => {
  ...
  const handleDraw = () => {
    ...
    setTimeout(() => {
      ...
      console.log(result);
      setImageSrc(DATA.activities[result].imageSrc + ".png");
      setResultText(DATA.activities[result].title);
  ...
  return (
    <div className="items-center justify-center h-[100vh] text-center bg-gray-950">
      ...
      {/* {isAnimating || resultText !== "" ? (
        ...
        <motion.div
          className={`absolute z-30 font-bold duration-200 ${
            versionExpand ? "top-20 right-20" : "top-10 right-10"
          } ${isVersionButtonPressed ? "scale-90" : "scale-100"}`}
        >
          <GameButton
            onClick={() => {
              setVersion(version > 3 ? 1 : version + 1);
            }}
            className={`px-10 h-20 rounded-full text-3xl duration-300 ${
              versionExpand
                ? "scale-120 bg-white opacity-100 text-black"
                : "scale-100 bg-black opacity-20 text-white"
            }`}
          >
